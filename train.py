...
parser.add_argument('--save_dir', type=str, help='saving directory',default="/weights/trash")
parser.add_argument('--file_dir', type=str, help='Training Val Dataset directory',default="/datasets/waymo110/preprocessed_data")
...
def _parse_image_function_traj(example_proto, with_traj = True):
    ...
    new_dict['gt_infos'] = tf.cast(tf.reshape(tf.io.decode_raw(d['gt_infos'],tf.float64),[64,3]),tf.float32)
...
if __name__ == "__main__":
    ...
    filenames = tf.io.matching_files(f'{args.file_dir}/train_trajs/*.tfrecords')
    ...
    train_dataset = train_dataset.map(_parse_image_function_traj)
    train_dataset = train_dataset.batch(BATCH_SIZE)

    v_filenames = tf.io.matching_files(f'{args.file_dir}/val_trajs/*.tfrecords')
    ...
    valid_dataset = valid_dataset.map(_parse_image_function_traj)
